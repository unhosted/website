/*
		Simple Planner
		A remoteStorage based personal planner prototype
		Made for 10K Apart
...
var Planner = {
	...
	init: function() {
		this.EVENTS = remoteStorage.getItem('events');
		if(!this.EVENTS) {
			this.EVENTS = {};
			if(!remoteStorage.getItem('setup')) {
				// initial setup
	...
	initUI: function() {
		...
		$('#btn-theme').click(function() {
			...
			remoteStorage.setItem('theme', t);
			
			Planner.theme(t);
			$(this).data('theme', t).html( Planner.UI.themes[ (t+1)%2 ] );
			return false;
		});
		if(remoteStorage.getItem('theme')) {
			Planner.theme(remoteStorage.getItem('theme'));
			$('#btn-theme').data('theme', remoteStorage.getItem('theme')).html( this.UI.themes[(parseInt(remoteStorage.getItem('theme'))+1) % 2] );
		} else {
	...
	deleteEvent: function(id, i) {
		...
		remoteStorage.setItem('events', JSON.stringify(this.EVENTS));
	},
